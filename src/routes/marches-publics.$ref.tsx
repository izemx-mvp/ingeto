...
function AssistantPanel({ market }: { market: PublicMarket }) {
  ...
  const ask = (question: string) => {
    ...
    setTimeout(
      () => {
        setTyping(false);
        setMessages((m) => [...m, { role: "bot", text: getAssistantReply(question, market) }]);
      },
      800 + Math.random() * 400,
    );
  };
  ...
  return (
    <>
      <motion.button
        ...
        className="fixed bottom-6 right-6 z-40 flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-medium text-primary-foreground shadow-[0_0_0_0_rgba(0,104,189,0.6)] transition-shadow hover:shadow-[0_0_36px_6px_rgba(0,104,189,0.45)]"
      >
          ...
          >
            <div className="flex items-center gap-3 border-b border-border px-4 py-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">{market.ref}</p>
              </div>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto p-4">
              <div className="flex gap-2">
                ...
                <div className="rounded-2xl rounded-tl-sm bg-muted px-3 py-2 text-sm">
                  Bonjour {USER.name.split(" ")[1]}, je connais l'intégralité du dossier {market.ref}.
          ...
          {stepIdx < MARKET_STEPS.length - 1 && (
            <Button
              onClick={() => {
                ...
                toast.success(`Dossier avancé à l'étape « ${MARKET_STEPS[stepIdx + 1]} »`);
              }}
            >
        ...
        <TabsContent value="synthese" className="mt-4 space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {[
              ...
              ["Date limite", fmtDate(market.deadline)],
